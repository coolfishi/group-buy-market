...
describe('商品筛选', () => {
  it('按分类与关键词过滤，无结果时返回空', () => {
    expect(filterProducts(products, '', 'jjk').map((p) => p.id)).toEqual(['JJ-01', 'JJ-02', 'JJ-03'])
    expect(filterProducts(products, 'pop up parade', 'all')).toHaveLength(5)
...
describe('演示模式完整流程', () => {
  it('开团 → 模拟支付 → 订单 → 退单 → 退款完成，刷新后保持', async () => {
    ...
    expect(res.order).toMatchObject({ status: 'PAY_WAIT', payAmount: 169 })
    ...
    expect((await again.getMarket('NR-01', user)).teams[0].isMine).toBe(true)
  ...
  it('拼团到期没凑齐：已付款订单自动退款', async () => {
    const { api, advance } = setup()
    const market = await api.getMarket('NR-01', user)
    const res = await api.checkout(user, { productId: 'NR-01', type: 'open', activityId: market.activityId })
    if (res.kind !== 'demo') throw new Error('expected demo')
    await api.settleDemoPayment(user, res.order.orderId, 'confirm')
    advance(25 * 3600_000)
    expect((await api.listOrders(user, null, 10)).orders[0]).toMatchObject({ status: 'WAIT_REFUND' })
    advance(9_000)
    expect((await api.listOrders(user, null, 10)).orders[0]).toMatchObject({ status: 'CLOSE', closeReason: '拼团到期未成团，已自动退款' })
  })
  ...
  it('参团补齐最后一人即拼团成功', async () => {
    ...
    const res = await api.checkout(user, { productId: 'JJ-01', type: 'join', teamId: team.teamId, activityId: market.activityId })
    if (res.kind !== 'demo') throw new Error('expected demo')
    const paid = await api.settleDemoPayment(user, res.order.orderId, 'confirm')
  ...
  it('取消支付会关闭订单并释放名额', async () => {
    const { api } = setup()
    const before = await api.getMarket('NR-02', user)
    const team = before.teams.find((t) => t.lockCount < t.targetCount)!
    const res = await api.checkout(user, { productId: 'NR-02', type: 'join', teamId: team.teamId, activityId: before.activityId })
    if (res.kind !== 'demo') throw new Error('expected demo')
    ...
    const after = await api.getMarket('NR-02', user)
    expect(after.teams.find((t) => t.teamId === team.teamId)?.lockCount).toBe(team.lockCount)
  })

  it('过期、满员、重复参团都会被拒绝', async () => {
    const { api } = setup()
    const expired = (await api.getMarket('JJ-02', user)).teams.find((t) => t.teamId.endsWith('EXP'))!
    await expect(api.checkout(user, { productId: 'JJ-02', type: 'join', teamId: expired.teamId })).rejects.toMatchObject({ code: 'E0102' })

    const full = (await api.getMarket('NR-01', user)).teams.find((t) => t.teamId.endsWith('FULL'))!
    expect(full.lockCount).toBe(full.targetCount)
    await expect(api.checkout(user, { productId: 'NR-01', type: 'join', teamId: full.teamId })).rejects.toMatchObject({ code: 'E0006' })

    const open = (await api.getMarket('JJ-03', user)).teams.find((t) => t.targetCount - t.lockCount >= 2)!
    await api.checkout(user, { productId: 'JJ-03', type: 'join', teamId: open.teamId })
    await expect(api.checkout(user, { productId: 'JJ-03', type: 'join', teamId: open.teamId })).rejects.toMatchObject({ code: 'E0103' })
  })

  it('同一商品提交中重复点击会被拦截', async () => {
    const api = createDemoApi({ storage: window.localStorage, latencyMs: 20 })
    const first = api.checkout(user, { productId: 'NR-03', type: 'single' })
    await expect(api.checkout(user, { productId: 'NR-03', type: 'single' })).rejects.toMatchObject({ code: 'DUPLICATE' })
    await expect(first).resolves.toMatchObject({ kind: 'demo' })
  })

  it('订单分页', async () => {
    const { api } = setup()
    for (let i = 0; i < 7; i++) await api.checkout(user, { productId: 'NR-03', type: 'single' })
  ...
  it('待支付订单可直接取消，已关闭订单不能退单', async () => {
    const { api } = setup()
    const res = await api.checkout(user, { productId: 'NR-03', type: 'single' })
    if (res.kind !== 'demo') throw new Error('expected demo')
