...
test('浏览筛选 → 登录 → 参团 → 模拟支付 → 订单 → 退单，刷新后保持', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '动漫手办' }).click()
  await expect(page.getByText('共 2 件')).toBeVisible()
  await page.getByRole('link', { name: /星轨旅人·澪/ }).first().click()

  // 未登录点击参团 → 登录 → 回到商品页并恢复购买意图
  await expect(page.getByRole('heading', { level: 1, name: '星轨旅人·澪' })).toBeVisible()
  await page.locator('#teams').getByRole('button', { name: '参与拼团' }).first().click()
  await expect(page).toHaveURL(/\/login\?redirect=/)
  await page.getByRole('button', { name: '用体验账号登录' }).click()
  await expect(page).toHaveURL(/\/products\/TS-2001/)
